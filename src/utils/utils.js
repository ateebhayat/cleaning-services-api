...
function isEmpty(str) {
  ...
  str = isJsonObj(str) && Object.keys(str)?.length === 0 ? '' : str;
  str = isJsonStr(str) && Object.keys(JSON.parse(str))?.length === 0 ? '' : str;
  return (
...
function trimAllFieldsInObjectAndChildren(obj) {
  return JSON.parse(
    (typeof obj == 'string' ? obj : JSON.stringify(obj)).replace(
      /"\s+|\s+"/g,
      '"',
    ),
  );
